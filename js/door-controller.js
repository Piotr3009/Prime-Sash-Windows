  ...
  function setupLiveWatchers() {
    ...
    function updateTransomUI() {
      ...
      if (isFrontDr && tt === 'arched') {
        var autoH = Math.round((parseInt((document.getElementById('d-width') || {}).value) || 900) / 2) + 80;
        if (hInput) { hInput.value = autoH; hInput.disabled = true; hInput.style.opacity = '0.5'; }
        if (hint) hint.textContent = 'Semi-circle: half the door width plus an 80mm straight springing — height set automatically (' + autoH + 'mm).';
      } else {
