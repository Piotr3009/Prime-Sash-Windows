/**
 * door-controller.js
 * Controls door window configurator menu — separate from sash specification-controller.js
...
(function() {
  ...
  // ─── Door colour state (full payload sent every time) ───
  var doorColourState = {
    sameColor: true,
    woodColor: '#F6F6F6',
    woodColorInt: '#F6F6F6',
  ...
  function setDefaultDimensions(layout) {
    ...
    const wSel = $('d-width-select');
    const hSel = $('d-height-select');
    const wHid = $('d-width');
    const hHid = $('d-height');
    const wCust = $('d-width-custom');
    const hCust = $('d-height-custom');
  ...
  function updateSpecPanel() {
    var layout = checked('door-layout') || '040L';
    var w = parseInt(val('d-width')) || 800;
    var h = parseInt(val('d-height')) || 1200;
    var isArched = checked('door-type') === 'arched';

    // Arched door overrides
    ...
    if (specSashType) specSashType.textContent = isArched
      ? 'Arched Door — ' + (shapeNames[archShape] || archShape) + ' (' + archHingeLabel + ')'
      : 'Door — Layout ' + layout;
    if (specSplitItem) specSplitItem.style.display = 'none';

    // Dimensions
    var specDims = document.getElementById('spec-dimensions');
    var specWidth = document.getElementById('sped-width');
    var specHeight = document.getElementById('sped-height');
    ...
    if (isArched) {
      ...
      var gothPat = checked('f-gothid-bars');
    ...
    var specBars = document.getElementById('sped-bars');
    if (specBars) {
    ...
    // Colour — read from door colour state
    ...
    // Glass specification (door-specific)
    var specCGlass = document.getElementById('spec-door-glass');
    if (specCGlass) {
    ...
    var specCSec = document.getElementById('spec-door-security');
    if (specCSec) {
    ...
    // Sill
    var specSill = document.getElementById('spec-door-sill');
    ...
    // Seal colour
    var specSeal = document.getElementById('spec-door-seal');
    if (specSeal) {
    ...
    if (infoPanel) {
      infoPanel.innerHTML =
        '<p class="info-title">Door Ordering Dimensions</p>' +
  ...
  function updateDoor3D() {
    ...
    window.update3D({
      ...
      doorHBars: checked('door-type') === 'arched' ? (parseInt(checked('f-hbars')) || 0) : (parseInt(checked('c-hbars')) || 0),
      doorVBars: checked('door-type') === 'arched' ? (parseInt(checked('f-vbars')) || 0) : (parseInt(checked('c-vbars')) || 0),
      doorOpening: (parseInt(val('c-opening')) || 0) / 100,
  ...
  function updateDoorPrice() {
    ...
    if (!config || config.windowType !== 'door') return;
  ...
  function setupColour() {
    ...
    cColorTypeRadios.forEach(function(radio) {
      radio.addEventListener('change', function() {
        ...
        if (radio.value === 'single') {
          ...
          doorColourState.sameColor = true;
          doorColourState.woodColorInt = doorColourState.woodColor;
          doorColourState.woodColorExt = doorColourState.woodColor;
        } else {
          ...
          // Switch to dual mode
          doorColourState.sameColor = false;
        }
    ...
    document.querySelectorAll('input[name="c-seal-colour"]').forEach(function(r) {
      r.addEventListener('change', updateDoorColour);
    });
  }
  ...
  function applyColourHex(hex, target) {
    ...
    if (target === 'single') {
      doorColourState.sameColor = true;
      doorColourState.woodColor = hex;
      doorColourState.woodColorInt = hex;
      doorColourState.woodColorExt = hex;
  ...
  function populateFBDropdowns() {
    ...
    if (!source) {
      console.log('F&B source dropdown not found — door FB dropdowns empty');
      return;
    }
  ...
  function setupLayoutChange() {
    var layoutRadios = document.querySelectorAll('input[name="door-layout"]');
  ...
  // ─── Live watchers for all door inputs ───
  function setupLiveWatchers() {
    // Dimensions
    setupDimSelect('d-width-select', 'd-width-custom', 'd-width');
    setupDimSelect('d-height-select', 'd-height-custom', 'd-height');

    // Fanlight ratio
    var fRatio = $('c-fanlight-height');
    if (fRatio) fRatio.addEventListener('change', updateDoor3D);

    // Bars
    document.querySelectorAll('input[name="c-hbars"], input[name="c-vbars"]').forEach(function(r) {
      r.addEventListener('change', updateDoor3D);
    });
    ...
    document.querySelectorAll('input[name="c-glass-finish"]').forEach(function(r) {
      r.addEventListener('change', function() { updateDoor3D(); updateSpecPanel(); });
    });
  ...
  // ─── Store door config (parallel to window.currentConfig for sash) ───
  function getDoorConfig() {
    var isArched = checked('door-type') === 'arched';
    return {
      windowType: 'door',
      windowCategory: 'door',
      doorType: checked('door-type') || 'standard',
      ...
      doorLayout: checked('door-layout') || '040L',
      layout: checked('door-layout') || '040L',
      width: parseInt(val('d-width')) || 800,
      height: parseInt(val('d-height')) || 1200,
      fanlightHeight: parseInt(val('c-fanlight-height')) || 350,
      doorHBars: isArched ? (parseInt(checked('f-hbars')) || 0) : (parseInt(checked('c-hbars')) || 0),
      doorVBars: isArched ? (parseInt(checked('f-vbars')) || 0) : (parseInt(checked('c-vbars')) || 0),
      ...
      fixGothicBars: isArched ? (checked('f-gothid-bars') || 'none') : 'none',
  ...
  function setup() {
    ...
    var currentLayout = checked('door-layout') || '040L';
