          ...
          <group onPointerOver={() => setHovered(true)} onPointerOut={() => setHovered(false)}>
            {config.windowCategory === 'casement' && config.casementType === 'arched' ? (
              ...
              <CasementWindow
                ...
                fanVBars={config.casementFanVBars || 0}
                fan2HBars={config.casementFan2HBars || 0}
                fan2VBars={config.casementFan2VBars || 0}
                opening={config.casementOpening || 0}
                fanlightRatio={config.fanlightRatio || 0.3}
                fan2Ratio={config.casementFan2Ratio || 0.3}
                woodColor={config.woodColor}
                woodColorExt={config.woodColorExt}
                woodColorInt={config.woodColorInt}
                sameColor={config.sameColor}
              ...
              <DoorWindow
                ...
                transomType={config.transomType || 'none'}
                transomHeight={config.transomHeight || 450}
                transomBars={config.transomBars || 'none'}
                doorStyle={config.doorStyle}
                centerMullion={config.centerMullion}
                paneling={config.paneling}
                panelGrid={config.panelGrid}
                furniture={config.furniture}
                fanBarPattern={config.fanBarPattern}
...
export default function App() {
  ...
  const [casementHinges, setCasementHinges] = useState(null);
  const [casementFan2Ratio, setCasementFan2Ratio] = useState(0.3);
  const [casementFanHBars, setCasementFanHBars] = useState(0);
  const [casementFanVBars, setCasementFanVBars] = useState(0);
  const [casementFan2HBars, setCasementFan2HBars] = useState(0);
  const [casementFan2VBars, setCasementFan2VBars] = useState(0);
  ...
  const [archProfile, setArchProfile] = useState('equilateral'); // gothic steepness (22.08.2026)
  const [lowerHBars, setLowerHBars] = useState(0);                // 'match' lower bars: horizontal count
  ...
  const [casementMiddleWidth, setCasementMiddleWidth] = useState(null);
  ...
  const [frontDoorLeaves, setFrontDoorLeaves] = useState(1); // 1 = single, 2 = double (50/50)
  ...
  const [panelGrid, setPanelGrid] = useState(null);
  const [furniture, setFurniture] = useState(null);
  const [fanBarPattern, setFanBarPattern] = useState('none');
  ...
  function restoreState(s) {
    ...
    if (s.glassType !== undefined) setGlassType(s.glassType);
    if (s.casementHinges !== undefined) setCasementHinges(s.casementHinges);
    if (s.casementFan2Ratio !== undefined) setCasementFan2Ratio(s.casementFan2Ratio);
    if (s.casementFanHBars !== undefined) setCasementFanHBars(s.casementFanHBars);
    if (s.casementFanVBars !== undefined) setCasementFanVBars(s.casementFanVBars);
    if (s.casementFan2HBars !== undefined) setCasementFan2HBars(s.casementFan2HBars);
    if (s.casementFan2VBars !== undefined) setCasementFan2VBars(s.casementFan2VBars);
    ...
    if (s.archProfile !== undefined) setArchProfile(s.archProfile);
    ...
    if (s.frontDoorLeaves !== undefined) setFrontDoorLeaves(s.frontDoorLeaves);
    ...
    if (s.panelGrid !== undefined) setPanelGrid(s.panelGrid);
    if (s.furniture !== undefined) setFurniture(s.furniture);
    if (s.fanBarPattern !== undefined) setFanBarPattern(s.fanBarPattern);
    ...
    if (s.transomType !== undefined) setTransomType(s.transomType);
    if (s.transomHeight !== undefined) setTransomHeight(s.transomHeight);
  ...
  React.useEffect(() => {
    window.update3D = (cfg) => {
      ...
      if (cfg.glassType    !== undefined) setGlassType(cfg.glassType);
      if (cfg.casementHinges !== undefined) setCasementHinges(cfg.casementHinges);
      if (cfg.casementFan2Ratio !== undefined) setCasementFan2Ratio(cfg.casementFan2Ratio);
      if (cfg.casementFanHBars !== undefined) setCasementFanHBars(cfg.casementFanHBars);
      if (cfg.casementFanVBars !== undefined) setCasementFanVBars(cfg.casementFanVBars);
      if (cfg.casementFan2HBars !== undefined) setCasementFan2HBars(cfg.casementFan2HBars);
      if (cfg.casementFan2VBars !== undefined) setCasementFan2VBars(cfg.casementFan2VBars);
      ...
      if (cfg.archProfile !== undefined) setArchProfile(cfg.archProfile);
      ...
      if (cfg.frontDoorLeaves !== undefined) setFrontDoorLeaves(parseInt(cfg.frontDoorLeaves, 10) === 2 ? 2 : 1);
      ...
      if (cfg.panelGrid !== undefined) setPanelGrid(cfg.panelGrid);
      if (cfg.furniture !== undefined) setFurniture(cfg.furniture);
      if (cfg.fanBarPattern !== undefined) setFanBarPattern(cfg.fanBarPattern);
  ...
  const config = useMemo(
    () => ({
      width,
      height: height + (windowCategory === 'door' && doorType === 'french' && transomType !== 'none' ? transomHeight : 0),
      extWidth,
      extHeight,
      opening,
      upperOpening,
      ...
      boxDepthLabel: glassType === 'triple' ? 172 : boxType === 'standard' ? 164 : 144,
      sashDepth: 57,
      boxType,
      glassType,
      casementHinges,
      ...
      upperMaxDrop: sashType === 'arched' ? archedSashMetrics(extWidth, extHeight, archShape, archProfile).upperMaxDrop : upperMaxDrop,
      lowerMaxLift: sashType === 'arched' ? archedSashMetrics(extWidth, extHeight, archShape, archProfile).lowerMaxLift : 0,
      archRise: sashType === 'arched' ? archedSashMetrics(extWidth, extHeight, archShape, archProfile).riseMm : 0,
      straightHeight: sashType === 'arched' ? archedSashMetrics(extWidth, extHeight, archShape, archProfile).straightHeightMm : 0,
      upperSashHeight: sashType === 'arched' ? Math.round(archedSashMetrics(extWidth, extHeight, archShape, archProfile).upperSashHeight) : 0,
      windowCategory,
      casementLayout,
      casementMiddleWidth,
      casementOpening,
