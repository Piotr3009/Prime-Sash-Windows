/**
 * FixFrameWindow.jsx — v3
 * Fix frame with double glazing + ovolo/chamfer beading on all shapes
 */
...
const SPACER_T = mm(1);
// Bead dimensions (matching casement)
const OVOLO_R = mm(8);     // interior ovolo bead tube radius
const CHAMFER_R = mm(5);   // exterior chamfer bead tube radius
const SPACER_R = mm(1.5);  // spacer strip tube radius
...
/* ─── Path from 2D points at given Z ─── */
function makeCurve3(pts2d, z) {
  const v = pts2d.map(p => new THREE.Vector3(p[0], p[1], z));
  return new THREE.CatmullRomCurve3(v, true, 'centripetal', 0.01);
}
...
/* ─── Double Glazing + Beads + Spacer ─── */
function GlazingUnit({ innerPts, glassShape, glassMat, spacerMat, frameMat, frameDepth }) {
  ...
  // Beads and spacer along inner perimeter
  const ovoloGeo = useMemo(() => {
    try {
      const curve = makeCurve3(innerPts, -halfD + OVOLO_R * 0.5);
      return new THREE.TubeGeometry(curve, innerPts.length * 2, OVOLO_R, 8, true);
    } catch(e) { return null; }
  }, [innerPts, halfD]);

  const chamferGeo = useMemo(() => {
    try {
      const curve = makeCurve3(innerPts, halfD - CHAMFER_R * 0.5);
      return new THREE.TubeGeometry(curve, innerPts.length * 2, CHAMFER_R, 6, true);
    } catch(e) { return null; }
  }, [innerPts, halfD]);

  const spacerGeo = useMemo(() => {
    try {
      const curve = makeCurve3(innerPts, 0);
      return new THREE.TubeGeometry(curve, innerPts.length * 2, SPACER_R, 6, true);
    } catch(e) { return null; }
  }, [innerPts]);
  ...
  return (
    <group>
      ...
      {/* Interior ovolo bead */}
      {ovoloGeo && <mesh geometry={ovoloGeo} castShadow receiveShadow><primitive object={frameMat} attach="material" /></mesh>}
      {/* Exterior chamfer bead */}
      {chamferGeo && <mesh geometry={chamferGeo} castShadow receiveShadow><primitive object={frameMat} attach="material" /></mesh>}
      {/* Spacer strip between panes */}
      {spacerGeo && <mesh geometry={spacerGeo} castShadow receiveShadow><primitive object={spacerMat} attach="material" /></mesh>}
    </group>
  );
}
...
function CircleFrame({ diameter, depth, mat, glassMat, spacerMat }) {
  ...
  const { frameGeo, innerPts, glassShape } = useMemo(() => {
    const outer = arcPoints(0, 0, R, 0, Math.PI * 2, segs);
    const inner = arcPoints(0, 0, rInner, 0, Math.PI * 2, segs);
    const glassR = rInner - mm(2);
    const glassPts = arcPoints(0, 0, glassR, 0, Math.PI * 2, segs);
    return {
      frameGeo: makeFrameGeo(outer, inner, D),
      innerPts: inner,
      glassShape: makeGlassShape(glassPts),
    };
  }, [R, rInner, D]);
  ...
  return (
    <group>
      ...
      <GlazingUnit innerPts={innerPts} glassShape={glassShape} glassMat={glassMat} spacerMat={spacerMat} frameMat={mat} frameDepth={depth} />
    </group>
  );
}
...
function GothicArchFrame({ width, height, depth, mat, glassMat, spacerMat, gothicBars = 'none' }) {
  ...
  const { frameGeo, innerPts, glassShape } = useMemo(() => {
    ...
    return { frameGeo: makeFrameGeo(outer, inner, D), innerPts: inner, glassShape: makeGlassShape(inner) };
...
function DimensionGuide({ from, to, label, offset = [0, 0, 0] }) {
  ...
  return (
    <group>
      ...
      <Text position={mid} fontSize={0.06} color="#22324a" anchorX="center" anchorY="middle" outlineColor="#f5f2ec" outlineWidth={0.008}>{label}</Text>
    </group>
  );
}
...
export default function FixFrameWindow({
  ...
  const extMaterial = useMemo(() => new THREE.MeshPhysicalMaterial({ color: cExt, roughness: 0.72, metalness: 0.02, clearcoat: 0.06, clearcoatRoughness: 0.4 }), [cExt]);
  const intMaterial = useMemo(() => new THREE.MeshPhysicalMaterial({ color: cInt, roughness: 0.72, metalness: 0.02, clearcoat: 0.06, clearcoatRoughness: 0.4 }), [cInt]);
  ...
  if (fixShape === 'circle')
    ...
    shapeNode = <CasementPanel width={width} height={height} hingeType="fixed" opening={0} material={extMaterial} materialInt={intMaterial} spacerColor={spacerColor} glassFinish={glassFinish} hBars={hBars} vBars={vBars} ironmongery="brass" position={[0,0,0]} />;
